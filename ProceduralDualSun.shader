// Adapted from Unity's built-in shader. Copyright (c) Unity Technologies.

Shader "Skybox/Procedural Dual-Sun" {
    Properties {
        [Header(Sky Settings)]
        [Space]
        ...
        [Header(First Sun)]
        [Space]
        [Toggle(FIRST_SUN)] _FirstSun("Enabled", Float) = 0
        _FirstSunSize("Size", Range(0,1)) = 0.04
        _FirstSunSizeConvergence("Convergence", Range(1,10)) = 5
        _FirstSunTint("Tint", Color) = (.5, .5, .5, 1)
        _FirstSunPosition("Position", Vector) = (0, 0, 0, 0)
        
        [Header(Second Sun)]
        [Space]
        [Toggle(SECOND_SUN)] _SecondSun("Enabled", Float) = 0
        _SecondSunSize("Size", Range(0,1)) = 0.04
        _SecondSunSizeConvergence("Convergence", Range(1,10)) = 5
        _SecondSunTint("Tint", Color) = (.5, .5, .5, 1)
        _SecondSunPosition("Position", Vector) = (0, 0, 0, 0)
    }
    ...
    SubShader {
        ...
        Pass {
            ...
            #pragma shader_feature FIRST_SUN
            #pragma shader_feature SECOND_SUN
            ...
            uniform half _FirstSunSize;
            uniform half _FirstSunSizeConvergence;
            uniform half3 _FirstSunTint;
            uniform half3 _FirstSunPosition;

            uniform half _SecondSunSize;
            uniform half _SecondSunSizeConvergence;
            uniform half3 _SecondSunTint;
            uniform half3 _SecondSunPosition;
            ...
            half getMiePhase(half eyeCos, half eyeCos2, float size) {
            ...
            half calcSunAttenuation(half3 lightPos, half3 ray, float size, float convergence) {
                #if SKYBOX_SUNDISK == SKYBOX_SUNDISK_SIMPLE
                    ...
                    return spot * spot;
                #else // SKYBOX_SUNDISK_HQ
                    half focusedEyeCos = pow(saturate(dot(lightPos, ray)), convergence);
            ...
            half4 frag(v2f IN) : SV_Target {
                ...
                col = lerp(IN.skyColor, IN.groundColor, saturate(y));
                    ...
                    if (y < 0.0) {
                        #ifdef FIRST_SUN
                            col += _FirstSunTint * calcSunAttenuation(normalize(_FirstSunPosition), -ray, _FirstSunSize, _FirstSunSizeConvergence);
                        #endif

                        #ifdef SECOND_SUN
                            col += _SecondSunTint * calcSunAttenuation(normalize(_SecondSunPosition), -ray, _SecondSunSize, _SecondSunSizeConvergence);
                        #endif
                    }
                #endif
